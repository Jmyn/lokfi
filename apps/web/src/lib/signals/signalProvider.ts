...
import { CredentialManager, DexieCredentialStore, createBrokerageProvider } from '../brokerage'
import { db } from '../db/db'

const credManager = new CredentialManager(new DexieCredentialStore(db))
const SUPPORTS_HISTORICAL_BARS = new Set(['tiger', 'cdc'])
...
async function resolveProvider(): Promise<{
  ...
  for (const cred of sorted) {
    ...
    try {
      ...
      const provider = createBrokerageProvider(source, plaintext)
      if (!provider) {
