...
import { ParseError, type Statement, type StatementParser, type StatementSource, type Transaction } from '../../types'
import { normalizeDate, parseAmount } from './csvUtils'
...
const AMT_KEYWORDS = new Set(['amount', 'transaction amount', 'amt', 'net amount', 'transaction_amount'])
const WDL_KEYWORDS = new Set(['withdrawal', 'debit', 'debit amount', 'dr', 'dr amount', 'charges'])
const DEP_KEYWORDS = new Set(['deposit', 'credit', 'credit amount', 'cr', 'cr amount'])
...
export class GenericCsvParser implements StatementParser {
  ...
  parse(text: string): Statement {
    ...
    if (headerRowIdx === -1) {
      for (let r = 0; r < Math.min(data.length, 15); r++) {
        ...
        for (let i = 0; i < row.length; i++) {
          ...
          const isDate =
            /^\d{2,4}[-/]\d{2}[-/]\d{2,4}(?:\s+\d{2}:\d{2}(:\d{2})?)?$/.test(val) ||
    ...
    for (let i = firstDataRowIdx; i < data.length; i++) {
      ...
      const descRaw = (descIdx !== -1 ? row[descIdx]?.trim() || 'Unknown' : 'Unknown').replace(/[\r\n]+/g, ' ')
      ...
      if (amtIdx !== -1) {
        ...
        if (wStr) {
          ...
          if (wVal !== null) {
            amtValue = -Math.abs(wVal)
            hasAmt = true
          }
        } else if (dStr) {
          const dVal = parseAmount(dStr)
          if (dVal !== null) {
            amtValue = Math.abs(dVal)
            hasAmt = true
          }
        }
      }
